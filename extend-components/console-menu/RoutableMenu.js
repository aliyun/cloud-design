 ...
 * 2. `import CompatibleMenu from '@alife/alicloud-components-console-menu/CompatibleMenu'`
 * RoutableMenu同理。
 * 注：为什么不直接让用户：
 * `import CompatibleMenu from '@alife/alicloud-components-console-menu/es/CompatibleMenu'`
 * 因为我们不希望用户依赖于我们的内部目录结构`es`，未来我们可能把构建结果放在`esnext`。
 *
 * 如果用户不使用这两个特殊组件，那么他可以直接
 * `import ConsoleMenu from '@alife/alicloud-components-console-menu'`，不用安装dva。
 */
